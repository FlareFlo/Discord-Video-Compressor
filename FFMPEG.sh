...
ffmpeg -i "${1}" -b:a "$audiobitrate" -c:v libx264 -vf scale="$horizontalresolution":"$verticalresolution" -preset "$(jq -r ".configs[0] | .preset" $(pwd)/$(jq -r ".configs[0] | .selection" main-config.json))" -crf "$crf" -fs "$(jq -r ".configs[0] | .size" $(pwd)/$(jq -r ".configs[0] | .selection" main-config.json))" "${1}"compressed.mp4